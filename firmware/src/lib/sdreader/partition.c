...
struct partition_struct* partition_open(device_read_t device_read, device_read_interval_t device_read_interval, device_write_t device_write, device_write_interval_t device_write_interval, int8_t index) {
	...
	// check if MBR exists
	bool mbr;
	if (!device_read(0x1e0, buffer, sizeof(buffer))) {
		return 0;
	}
	if (buffer[0x1e] == 0x55 && buffer[0x1f] == 0xAA) {
		MSG("mbr found");
		mbr = true;
	} else {
		MSG_HEX("-1: ", buffer[0], 1);
		MSG_HEX("00: ", buffer[1], 1);
		MSG("not mbr");
		// MBR not found
		if (!device_read(0, buffer, sizeof(buffer))) {
			return 0;
		}
		if (buffer[0x0B] == 0 && buffer[0x0C] == 2 && buffer[0x11] == 0 && (buffer[0x12] == 0 || buffer[0x12] == 2) && buffer[0x15] == 0xF8) {
			mbr = false;
			// FAT16/32, superfloppy
		} else {
			return 0;	// TODO need error codes here
		}
	}
    ...
    if (index >= 0) {
		 if (mbr) {
			...
			new_partition->has_mbr = true;
			MSG_HEX("type(mbr) ", new_partition->type, 1);
			if (new_partition->type != PARTITION_TYPE_FAT12 && new_partition->type != PARTITION_TYPE_FAT16_32MB && 
					  new_partition->type != PARTITION_TYPE_FAT16 && new_partition->type != PARTITION_TYPE_FAT32 && 
					  new_partition->type != PARTITION_TYPE_FAT32_LBA && new_partition->type != PARTITION_TYPE_FAT16_LBA) {
				MSG_HEX("WRONG PARTITION TYPE ", new_partition->type, 1);
			} else {
				return new_partition;
			}
		 }
		 //} else {
			new_partition->type = buffer[0x12] == 0 ? PARTITION_TYPE_FAT32 : PARTITION_TYPE_FAT16;
			new_partition->offset = 0;
			new_partition->length = 0x100;	// TODO !!! это поле нигде не используется, поэтому такой костыль работает
			new_partition->has_mbr = false;
			MSG_HEX("type ", new_partition->type, 1);
		 //}
    } else {
