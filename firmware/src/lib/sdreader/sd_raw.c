...
#if TROL_SPEED_INLINE

inline uint8_t sd_raw_rec_byte_m() {
	// send dummy data for receiving some
	SPDR = 0xff;
	while (!(SPSR & _BV(SPIF)));
	SPSR &= ~ _BV(SPIF);
...
#else
	#define sd_raw_rec_byte_m		sd_raw_rec_byte
#endif
...
uint8_t sd_raw_read(offset_t offset, uint8_t* buffer, uintptr_t length) {
    ...
    while (length > 0) {
        ...
        if (block_address != raw_block_address || trol_fast_read_mode)
#endif
        {
#if SD_RAW_WRITE_BUFFERING
            if (!sd_raw_sync())
...
#if SD_RAW_SDHC
//MSG_DEC("raw-block-address ", block_address);				
            ...
            // wait for data block (start byte 0xfe)
				uint16_t waitCnt = 0;
            while (sd_raw_rec_byte_m() != 0xfe) {
					if (++waitCnt == 0) {
						// TODO добавил сюда этот таймаут надо посмотреть, насколько он достаточен !!!
						unselect_card();
						return 0;
					}
				};
            ...
            for(uint16_t i = 0; i < 512; ++i) {
                uint8_t b = sd_raw_rec_byte_m();
                if (i >= block_offset && i < read_to) {
                    *buffer++ = b;
				}
            }
#else
			if (trol_fast_read_mode) {
				if (block_offset == 0 && read_length == 512) {
					for (uint16_t i = 0; i < 512; ++i) {
						*buffer++ = sd_raw_rec_byte_m();
					}
				} else {
					uint16_t read_to = block_offset + read_length;
					for (uint16_t i = 0; i < 512; ++i) {
...
uint8_t sd_raw_read_interval(offset_t offset, uint8_t* buffer, uintptr_t interval, uintptr_t length, sd_raw_read_interval_handler_t callback, void* p) {
    ...
    while(length >= interval) {
        // as reading is now buffered, we directly hand over the request to sd_raw_read()
...
uint8_t sd_raw_write(offset_t offset, const uint8_t* buffer, uintptr_t length) {
    ...
    while (length > 0) {
        ...
        // Merge the data to write with the content of the block. Use the cached block if available.        
