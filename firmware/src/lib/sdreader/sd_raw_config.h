...
/**
 *
 * Set to 1 to use inline functions and increase the speed (and code size too)
 */
#define TROL_SPEED_INLINE	1
...
/* defines for customisation of sd/mmc port access */
	...
	#define configure_pin_ss() DDRC |= _BV(0)

    #define configure_pin_miso() DDRB &= ~(1 << DDB3)

    //#define select_card() PORTD &= ~(1 << PORTD7)
    //#define unselect_card() PORTD |= (1 << PORTD7)
    #define select_card() PORTC &= ~_BV(0)
    #define unselect_card() PORTC |= _BV(0)
...
#define configure_pin_available() DDRD &= ~_BV(7); PORTD |= _BV(7);//DDRC &= ~(1 << DDC4)
#define configure_pin_locked() DDRB &= ~_BV(7); PORTB |= _BV(7);//DDRC &= ~(1 << DDC5)

#define get_pin_available() (PIND & _BV(7))	//0			//(PINC & (1 << PINC4))
#define get_pin_locked() (PINB & _BV(7))		//1         //(PINC & (1 << PINC5))
